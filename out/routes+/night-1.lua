...
do
	...
	function LootableComponent:constructor(instance)
		...
		local root = instance:WaitForChild("Handle", 30)
		if not root then
		...
		label.Center = true
		label.Outline = true
		local indicator = Drawing.new("Quad")
	...
	function LootableComponent:onConfigure()
		local _binding = self
		local bin = _binding.bin
		local root = _binding.root
		local label = _binding.label
		local indicator = _binding.indicator
		local renderer = _binding.renderer
		...
		local connection = RunService.RenderStepped:Connect(function()
			...
			local viewport, onScreen = CurrentCamera:WorldToViewportPoint(position)
			if onScreen then
				local _binding_1 = viewport
				local x = _binding_1.X
				local y = _binding_1.Y
				label.Position = Vector2.new(x, y)
				indicator.PointA = Vector2.new(x, y + 4)
				indicator.PointB = Vector2.new(x + 4, y)
				indicator.PointC = Vector2.new(x, y - 4)
				indicator.PointD = Vector2.new(x - 4, y)
				label.Visible = true
...
do
	...
	local window = DisplayLibrary.window()
	local Mutant = {}
	do
